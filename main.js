...
import { MeshCollisionSystem } from './meshCollision.js';
import { ModelInfoViewer } from './modelInfoViewer.js';
import { Minimap } from './Minimap.js';
import { MobileControls } from './mobileControls.js';
let mobileControls = null;
let crosshair, timeDisplay; // Add these global declarations
...
let scene, galleryScene, cubeViewerScene, sphereViewerScene, cylinderViewerScene, churchViewerScene, coneViewerScene, activeScene;
...
try {
  ...
  cubeViewerScene = new THREE.Scene();
  sphereViewerScene = new THREE.Scene();
  cylinderViewerScene = new THREE.Scene();
  churchViewerScene = new THREE.Scene();
  coneViewerScene = new THREE.Scene(); 
  activeScene = scene;
...
try {
  ...
  setTimeout(() => {
    if (meshCollisionSystem.collisionMeshes.size > 0) {
      console.log('Auto-enabling collision debug visualization...');
      meshCollisionSystem.toggleDebugMode(scene);
    }
  }, 5000); // Increased delay to allow for positioning
...
function checkCollision(currentPosition, newPosition) {
  ...
  // Check mesh-based collision - simple true/false, no sliding
  const hasCollision = meshCollisionSystem.checkCollision(currentPosition, newPosition, characterRadius);
  
  if (hasCollision) {
    // Don't modify newPosition - just return true to block movement
    return true;
  }

  // Keep existing manual collision boxes as fallback
  for (const collisionBox of collisionBoxes) {
    ...
    if (boxGeometry.type === 'CylinderGeometry') {
      // Cylinder collision logic (unchanged)
...
function getGroundLevel(position) {
  ...
  // NEVER use mesh collision for ground level - only use manual collision boxes
  let groundLevel = 0;
  
  // Check manual collision boxes as fallback only
  for (const collisionBox of collisionBoxes) {
    const boxGeometry = collisionBox.geometry;
    const boxPosition = collisionBox.position;
    if (boxGeometry.type === 'CylinderGeometry') {
...
function moveCharacter(dt) {
  ...
  if (characterSystem && characterSystem.isLoaded) {
    ...
    // Test X movement (horizontal only - preserve Y position)
    const testPositionX = currentPos.clone();
    testPositionX.x += desiredVelocity.x;
    // Keep Y the same for collision testing
    testPositionX.y = currentPos.y;
    if (!checkCollision(currentPos, testPositionX)) {
      characterGroup.position.x = testPositionX.x;
    }
    
    // Test Z movement (horizontal only - preserve Y position)
    const testPositionZ = currentPos.clone();
    testPositionZ.z += desiredVelocity.z;
    // Keep Y the same for collision testing
    testPositionZ.y = currentPos.y;
    if (!checkCollision(currentPos, testPositionZ)) {
      characterGroup.position.z = testPositionZ.z;
    }
    
    // Try diagonal movement if both X and Z are blocked separately
    if (Math.abs(desiredVelocity.x) > 0.01 && Math.abs(desiredVelocity.z) > 0.01) {
      const testPositionXZ = currentPos.clone();
      testPositionXZ.x += desiredVelocity.x;
      testPositionXZ.z += desiredVelocity.z;
      if (!checkCollision(currentPos, testPositionXZ)) {
        characterGroup.position.x = testPositionXZ.x;
        characterGroup.position.z = testPositionXZ.z;
      }
    }
    
    // Handle gravity and jumping independently of collision meshes
    if (isJumping || characterGroup.position.y > 0) {
      jumpVelocity -= gravity * dt;
      characterGroup.position.y += jumpVelocity * dt;
    }
    
    // Only use manual collision boxes for ground level - NEVER mesh collision
